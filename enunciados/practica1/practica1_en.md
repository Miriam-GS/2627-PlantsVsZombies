...
- [1. Description of the assignment](#1-descripción-de-la-práctica)
  ...
  * [1.2. Details of the assignment](#12-detalles-sobre-la-práctica)
    + [Entities in the game](#objetos-del-juego)
- [2. Game design](#2-organización-del-juego)
  * [2.1 Draw](#21-draw)
...
- **Sunflower**: Generates sun (the coin of the game)
...
For interest, here is a [more extensive list of plants](https://plantsvszombies.wiki.gg/wiki/Plants_(PvZ)) and a [more extensive list of zombies](https://plantsvszombies.wiki.gg/wiki/Zombies_(PvZ)).

![Catálogo de plantas del juego original](imgs/plants_vs_zombies_almanaq.jpg)

In the original game, each plant has a cost which is measured in sun. Sun can be obtained from certain types of plants or from the sky. The player can accumulate sun and use it to grow new plants.
...
The number of lives remaining (i.e. the endurance minus the number of damage points received so far) appears in square brackets next to each entity (plant or zombie) on the board. The output finishes with the game prompt, `Command >`, prompting the user to enter the next action.
...
- `list`: show the available plants with information about their cost and the damage they inflict. In this version, the output should be as follows:
...
- `help`: display the help message for each of the commands; each such help message comprises the name of a command, followed by a colon, followed by a brief description of what the command does. In this version, the output should be as follows:
...
- After the `exit` command, the game loop terminates before reaching any other phases.
...
must always be consulted in the same order:

1. the random number generator is accessed to choose the row on which the zombie may appear,
2. the random number generator is accessed to decide whether or not the zombie will actually appear in the game (using the probability for the current level).

Both consultations must be made on each cycle. If this procedure is not followed, the output may never exactly coincide with the expected output of the tests, in which case it will always be considered incorrect by the tests.
...
3. The Zombies advance if they can; a zombie does not advance if it has another zombie or a plant in its way.
4. If a Zombie is adjacent to a plant (on the next tile to the left in the same row) **and has not advanced in step 3** due
to being blocked by the plant, it inflicts damage on that plant.
5. If a plant or a Zombie has 0 remaining lives, it is removed from the board (in the original game, plants get eaten while zombies turn to ashes but in this version they both simply disappear).

The game finishes if, during an update, all the zombies are destroyed or one of the zombies goes off the l.h.s of the board. When the game terminates, the appropriate message, one of `Player wins` or `Zombies win`, must be displayed on the screen.

Within each category (Sunflowers, Peashooters, Zombies), the individual elements are to be updated in the order in which they were introduced into the game. Note that the state of the game at the end of the *update* phase of a cycle may depend on the update order; we have chosen one particular order so that all solutions will have the same behaviour in this regard.
...
The program is to be started by executing the class `pvz.PlantsVsZombies`, for which reason you are advised to place all the classes developed in the assignment in the package `pvz` (or subpackages of this package). Your implementation must contain at least the following classes:

- `Sunflower`, `Peashooter`, `Zombie`: These three classes encapsulate the behaviour of the entities of the game, which, in standard terminology are referred to as *game objects* (though this may cause confusion since sometimes it is used to refer to individual entities and sometimes to classes of such entitites). Game objects have attributes such as their position `x, y`, the number of remaining lives, etc. They also have an attribute in which they store the game, that is, an instance of the class `Game`, in order for objects of these classes to use methods of the `Game` class to check whether or not they can perform a given action.
...
- `GamePrinter`: This class has a `toString` method to print the current state of the game as shown above. It will need to access methods of the `game` to do so. It implements an interface, called `GameView`, but this is not relevant for the first assignment.
It needs (at least), the following two private attributes:
  ```java
  private Game game;
  private Scanner scanner;
  ```
  The object of class `Scanner` is used to read the user commands from the standard input (by default, the keyboard). 

- `Controller`: This class contains the `run` method which in turn contains the main loop controlling the execution of the game. Until the game terminates, this loop executes the following:
1. prompt the player to enter a command (via a call to a method of the `GamePrinter` class),
2. execute the command (via a call to a method of the `Game` class)
3. if appropriate, print the new state of the game (via a call to a method of the `GamePrinter` class).
The `Controller` class has an attribute to store an element of the `Game` class. 
...
If you wish, you may also include a folder called `doc` containing the API documentation in HTML format generated automatically from the Java source code of your solution using the *javadoc* tool.
...
Together with the skeleton of the assignment code, you will find a folder containing two types of (black-box) tests
to check that the behaviour of your code is as expected:

- Unit tests: these check the behaviour of small parts of the code, concretely, some methods of a few of the classes
- System tests: those provided check the input-output behaviour of the whole program (for a given input, i.e. series
of commands, provided on the standard input of the executing program, the output on the standard output of the
executing program is as expected).
...
## Unit tests
Unit tests of code used in the first assignment can be found in he directory `src/test/`.

## System tests
In the directory `src/test/resources`, you will find pairs of files with the following nomenclature:

- `00-easy_25-input.txt`: the input of test case `0`, in which the level is `easy` and seed has value `25`.
